...
async function formRecebimento(tipoDevedor, pessoa, titulosIniciais) {
  ...
  /* Ordem em que o dinheiro é distribuído: primeiro as parcelas que o usuário
     abriu (ele clicou nelas de propósito), depois as demais, da mais antiga
     para a mais nova. */
  const ordemAloc = [...todos].sort((a, b) =>
    (preSel.has(b.id) ? 1 : 0) - (preSel.has(a.id) ? 1 : 0)
  ...
  const m = modal({ titulo:`Registrar recebimento · ${pessoa.nome}`, largura:'wide',
    corpo:`${ehRev && pecas.length ? `<div class="tabs" id="rc_tabs" style="margin-bottom:16px">
      ...
      <div><label>Valor recebido <span style="color:var(--red)">*</span></label>
        <input class="inp num" type="number" step="0.01" min="0.01" id="rc_valor" value="${totalPre.toFixed(2)}">
        <div class="hint">Recebeu menos que a parcela? Digite o valor que entrou — o sistema
          distribui e a parcela fica <b>parcial</b>, com o resto em aberto.</div></div>
    ...
    <div class="tw" style="max-height:300px;overflow-y:auto;border:1px solid var(--line);border-radius:9px">
      <table class="itens-tb" id="rctb"><thead><tr>
        <th>Parcela</th><th>Vencimento</th><th class="r">Saldo</th><th style="width:20%">Alocar</th>
        <th class="r">Fica devendo</th><th class="c">Resultado</th></tr></thead><tbody></tbody></table></div>
  ...
  const render = () => {
    $('#rctb tbody', m.body).innerHTML = todos.map(t => {
      ...
      return `<tr data-id="${t.id}">
        ...
        <td class="r money ${rest > 0 && al ? 'neg' : ''}">${al ? BRL(rest) : '—'}</td>
  ...
  /* Atualiza só os valores dos campos de alocação, sem recriar a tabela —
     recriar tiraria o foco de quem está digitando o valor recebido. */
  const atualizarCampos = () => {
    $$('#rctb tbody tr', m.body).forEach(tr => {
      const inp = $('.al', tr); if (!inp || inp === document.activeElement) return;
      const al = N(aloc[tr.dataset.id]);
      inp.value = al ? al.toFixed(2) : '';
    });
  };
  const render2 = () => {
    $$('#rctb tbody tr', m.body).forEach(tr => {
      const t = todos.find(x => x.id === tr.dataset.id), al = N(aloc[t.id]), rest = N(t.saldo) - al;
      tr.children[4].className = 'r money' + (rest > 0 && al ? ' neg' : '');
      tr.children[4].textContent = al ? BRL(rest) : '—';
      tr.children[5].innerHTML = !al ? '<span class="tag n">—</span>' : rest <= 0 ? '<span class="tag g">✓ quitada</span>' : '<span class="tag a">parcial</span>';
    });
  };
  const resumo = () => {
  ...
  const auto = (redesenhar = true) => {
    let resta = Math.round(N($('#rc_valor', m.body).value) * 100) / 100;
    ordemAloc.forEach(t => {
      const usa = Math.max(0, Math.min(resta, N(t.saldo)));
      aloc[t.id] = Math.round(usa * 100) / 100;
      resta = Math.round((resta - usa) * 100) / 100;
    });
    if (redesenhar) render(); else { render2(); resumo(); }
  };
  ...
  /* Mudar o valor recebido redistribui sozinho. Sem isso, digitar um valor
     menor deixava a alocação antiga maior que o recebido e o botão travava
     em "excede o recebido" — que era o que impedia registrar pagamento
     parcial. Quem quiser dividir à mão edita as linhas depois. */
  $('#rc_valor', m.body).oninput = () => { auto(false); atualizarCampos(); };
