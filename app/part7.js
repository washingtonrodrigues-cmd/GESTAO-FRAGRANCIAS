...
async function abrirRelatorio(v, id) {
  ...
  let dados = [], colunas = [], totais = '';
  const carregar = async () => {
    const i = $('#r1').value, f = $('#r2').value;
    $('#rConteudo').innerHTML = CARREGANDO;
    const r = await montarRelatorio(id, i, f);
  ...
  $('#rPrint').onclick = () => imprimir(docRelatorio(cfg.t, $('#r1').value, $('#r2').value, colunas, dados, totais));
  await carregar();
}

async function montarRelatorio(id, i, f) {
